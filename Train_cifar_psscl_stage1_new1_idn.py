...
import dataloader_cifar_idn as dataloader
import pdb
import io
import PIL
...
parser.add_argument('--noise_mode', default='2')    # 1,2 for plc or pdl
...
if __name__ == '__main__':
    ...
    loader = dataloader.cifar_dataloader(args.dataset, r=args.r, noise_mode=args.noise_mode, batch_size=args.batch_size,
                                         ...
                                         noise_file='noise/plc/%s-%s-%.2f.json' % (args.dataset, args.noise_mode,args.r))
    ...
    for epoch in range(resume_epoch, args.num_epochs + 1):
        ...
        if epoch < warm_up:
            ...
            start_time = time.time()
            print('Train Net1')
            labeled_trainloader, unlabeled_trainloader, _ = loader.run('train', pred2, prob2, second_ind=second_ind)  # co-divide
            train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader, savelog=True)  # train net1

            print('\nTrain Net2')
            labeled_trainloader, unlabeled_trainloader, u_map_trainloader = loader.run('train', pred1, prob1, second_ind=second_ind)  # co-divide
