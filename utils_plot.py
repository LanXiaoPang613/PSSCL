...
def plot_histogram_loss_pred(data, inds_clean, inds_noisy, path, epoch):
    ...
    plt.hist(data[inds_clean],bins=bins, range=(0., 1.), edgecolor='black', alpha=0.5,
             label='clean - %d (%.1f%%)'%(num_inds_clean,perc_clean))

    if len(inds_noisy) >0:
        plt.hist(data[inds_noisy], bins=bins, range=(0., 1.), edgecolor='black', alpha=0.5,
                 label='noisy- %d (%.1f%%)'%(num_inds_noisy,100-perc_clean))
...
def plot_model_view_histogram_loss(data, idx_view_labeled, idx_view_unlabeled, inds_clean, inds_noisy, path, epoch):
    ...
    plt.hist(data[idx_view_labeled],bins=bins, range=(0., 1.), edgecolor='black', alpha=0.5,
             label='alg_view_clean(%d| %.1f%%)'%(num_view_labeled,100*num_view_labeled/float(total)))

    plt.hist(data[idx_view_unlabeled], bins=bins, range=(0., 1.), edgecolor='black', alpha=0.5,
             label='alg_view_noisy(%d| %.1f%%)'%(num_view_unlabeled,100*num_view_unlabeled/float(total)))

    plt.hist(data[missed_clean],bins=bins, range=(0., 1.), edgecolor='black', alpha=0.5,
             color='#fb8072', label='FN (%d| %.1f%%)'%(len(missed_clean),100*len(missed_clean)/float(len(inds_clean))))

    if len(inds_noisy) >0:
        plt.hist(data[missed_noisy],bins=bins, range=(0., 1.), edgecolor='black',
                 alpha=0.5, color='k',label='FP (%d| %.1f%%)'%(len(missed_noisy),100*len(missed_noisy)/float(len(inds_noisy))))
    plt.xlabel('Loss');
    plt.ylabel('Number of data')
    plt.legend( bbox_to_anchor=(0., 1.02, 1., .102), loc='lower left',
        ncol=2, mode="expand", borderaxespad=0.)
...
def plot_model_view_histogram_pred(data, idx_view_labeled, idx_view_unlabeled, inds_clean, inds_noisy, path, epoch):
    ...
    plt.hist(data[missed_clean],bins=bins, range=(0., 1.), edgecolor='black', alpha=0.5, color='#fb8072',
             label='FN (%d| %.1f%%)'%(len(missed_clean),100*len(missed_clean)/float(len(inds_clean))))

    if len(inds_noisy) >0:
        plt.hist(data[missed_noisy],bins=bins, range=(0., 1.), edgecolor='black', alpha=0.5, color='k',
                 label='FP (%d| %.1f%%)'%(len(missed_noisy),100*len(missed_noisy)/float(len(inds_noisy))))
    plt.xlabel('Pred');
    plt.ylabel('Number of data')
    plt.legend( bbox_to_anchor=(0., 1.02, 1., .102), loc='lower left',
        ncol=2, mode="expand", borderaxespad=0.)
